...
namespace Garfield.DAL
{
	...
	public partial class CRM_contract
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Garfield.Model.CRM_contract model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Garfield.Model.CRM_contract model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public Garfield.Model.CRM_contract GetModel(int id)
		{
			...
			Garfield.Model.CRM_contract model=new Garfield.Model.CRM_contract();
        ...
        /// <returns></returns>
        public DataSet Compared_empcuscontract(DateTime dt1, DateTime dt2, string idlist)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select hr_employee.name as yy,");
            strSql.Append(" SUM(case when YEAR( CRM_contract.Create_time)=YEAR('" + dt1 + "') and MONTH(CRM_contract.Create_time)=month('" + dt1 + "') then 1 else 0 end) as dt1, ");
            strSql.Append(" SUM(case when YEAR( CRM_contract.Create_time)=YEAR('" + dt2 + "') and MONTH(CRM_contract.Create_time)=month('" + dt2 + "') then 1 else 0 end) as dt2 ");
