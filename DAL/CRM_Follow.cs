...
namespace Garfield.DAL
{
	...
	public partial class CRM_Follow
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Garfield.Model.CRM_Follow model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Garfield.Model.CRM_Follow model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public Garfield.Model.CRM_Follow GetModel(int id)
		{
			...
			Garfield.Model.CRM_Follow model=new Garfield.Model.CRM_Follow();
        ...
        /// <returns></returns>
        public DataSet Compared_follow(DateTime dt1, DateTime dt2)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select Follow_Type as yy, count(Follow_Type)as xx,");
            strSql.Append(" SUM(case when YEAR( Follow_date)=YEAR('" + dt1 + "') and MONTH(Follow_date)=month('" + dt1 + "') then 1 else 0 end) as dt1, ");
            strSql.Append(" SUM(case when YEAR( Follow_date)=YEAR('" + dt2 + "') and MONTH(Follow_date)=month('" + dt2 + "') then 1 else 0 end) as dt2 ");
            strSql.Append(" FROM CRM_Follow group by Follow_Type");

            return DbHelperSQL.Query(strSql.ToString());
        }

        public DataSet Compared_empcusfollow(DateTime dt1, DateTime dt2, string idlist)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select hr_employee.name as yy,");
            strSql.Append(" SUM(case when YEAR( CRM_Follow.Follow_date)=YEAR('" + dt1 + "') and MONTH(CRM_Follow.Follow_date)=month('" + dt1 + "') then 1 else 0 end) as dt1, ");
            strSql.Append(" SUM(case when YEAR( CRM_Follow.Follow_date)=YEAR('" + dt2 + "') and MONTH(CRM_Follow.Follow_date)=month('" + dt2 + "') then 1 else 0 end) as dt2 ");
